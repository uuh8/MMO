...
namespace GameServer.Services
{
    class QuestService : Singleton<QuestService>
    {
        ...
        private void OnQuestAccept(NetConnection<NetSession> sender, QuestAcceptRequest message)
        {
            ...
            Log.InfoFormat("[QuestService] QuestAcceptRequest: character:{0}:QuestId{1}", character.Id, message.QuestId);
        ...
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("[QuestService] QuestSubmitRequest: character:{0}:QuestId{1}", character.Id, message.QuestId);
